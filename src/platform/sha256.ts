/**
 * Portable synchronous SHA-256 (FIPS 180-4).
 *
 * The accepted payload-hashing contract is synchronous (`hashPayload` returns
 * a hex digest). Web Crypto and expo-crypto expose only async digests, so the
 * algorithm itself is implemented here on `Uint8Array` with no `node:crypto`
 * or `Buffer` coupling. Canonicalization rules live in `sync/hashing.ts` and
 * are unchanged: this module only hashes an already-canonical UTF-8 string.
 */
...
/** SHA-256 digest of raw bytes, returned as 32 bytes. */
export function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  const padZero = (64 - ((withOne + 8) % 64)) % 64;
  const padded = new Uint8Array(withOne + padZero + 8);
  padded.set(message);
...
export function sha256Utf8Hex(message: string): string {
